...
stop = (np.abs(timing-vertical[15])).argmin();
stop2 = (np.abs(bstart-vertical[15])).argmin()

fig, ax = plt.subplots(2,1,sharex=True, sharey=False)
#fig.tight_layout()
ax[0].errorbar(timing[0:stop], y[0:stop],err[0:stop],ecolor='g')
#ax[0].vlines(flares,0,0.6,colors='r',linestyles='dashed')
ax[0].vlines(vertical[0:16],0,0.6,colors='r',linestyles='dashed')

ax[0].annotate('14702',xy=(0,1),xytext=(0,0.6),fontsize=10)


j=0
for j in range(0,stop2):
    if j < len(rates)-1:
       ...
       ax[1].bar(bstart[j]-bstart[0],rates[j], width=(bend[j]-bstart[j]),
                alpha=1,color='r')
plt.xlabel("Time(s)")
#plt.ylabel("Count rates (count/s)")

for ax in ax:
    ax.set_ylabel('Count rates (count/s)')


fig, ax2 = plt.subplots(2,1,sharex=True, sharey=False)
ax2[0].errorbar(timing[stop:], y[stop:],err[stop:],ecolor='g')
#ax[0].vlines(flares,0,0.6,colors='r',linestyles='dashed')
ax2[0].vlines(vertical[15:],0,0.6,colors='r',linestyles='dashed')

for j in range(stop2-1,len(rates)):
    if j < len(rates)-1:
        ax2[1].bar(bstart[j]-bstart[0],rates[j], width=(bstart[j+1]-bstart[j]),
                alpha=1,color='r')
    else:
       ax2[1].bar(bstart[j]-bstart[0],rates[j], width=(bend[j]-bstart[j]),
                alpha=1,color='r')
...
for ax2 in ax2:
    ax2.set_ylabel('Count rates (count/s)')
